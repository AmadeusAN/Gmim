...
from gmim_models.ssl_head import SSLHead
...
def main():
    ...
    def train(args, global_step, train_loader, val_best, scaler):
        ...
        for step, batch in enumerate(train_loader):
            ...
            with autocast(enabled=args.amp):
                ...
                loss, contrastive_loss, rec_loss = loss_function(
                    contrastive1_x, contrastive1_y, rec_x1, x
                )
            ...
            writer.add_scalar(
                "train/loss_total", scalar_value=loss.item(), global_step=global_step
            )
            writer.add_scalar(
                "train/loss_recon",
            ...
            writer.add_scalar(
                ...
                scalar_value=contrastive_loss.item(),
                global_step=global_step,
            )
                ...
                if dist.get_rank() == 0:
                    print(
                        "Step:{}/{}, Loss:{:.4f}, Time:{:.4f}".format(
                            global_step, args.num_steps, loss.item(), time() - t1
                        )
                    )
            else:
                print(
                    "Step:{}/{}, Loss:{:.4f}, Rcloss:{:.4f}, Conloss:{:.4f},Time:{:.4f}".format(
                        global_step,
                        args.num_steps,
                        loss.item(),
                        rec_loss.item(),
            ...
            if val_cond:
                ...
                writer.add_scalar(
                    ...
                    scalar_value=val_loss_recon.item(),
                    global_step=global_step,
                )
                writer.add_scalar(
                    "Validation/loss_recon",
                    scalar_value=val_loss.item() - val_loss_recon.item(),
                    global_step=global_step,
                )
    ...
    def validation(args, test_loader):
        ...
        with torch.no_grad():
            for step, batch in enumerate(test_loader):
                ...
                with autocast(enabled=args.amp):
                    contrastive1_x, contrastive1_y, rec_x1 = model(
                        val_inputs, val_inputs_y
                    )
                    if args.invis_patches:
                        val_inputs = model.swinViT.mask_layer.get_masked_gt(val_inputs)
                        rec_x1 = model.swinViT.mask_layer.get_masked_gt(rec_x1)
                    loss, contrastive_loss, rec_loss = loss_function(
                        contrastive1_x, contrastive1_y, rec_x1, val_inputs
                    )
                loss_val.append(loss.item())
                loss_val_recon.append(rec_loss.item())
                print(
                    "Validation step:{}, Loss:{:.4f}, Recloss:{:.4f}, Conloss:{:.4f}".format(
                        step,
                        loss.item(),
                        rec_loss.item(),
                        loss.item() - rec_loss.item(),
    ...
    parser.add_argument(
        "--dropout_path_rate",
        default=config.vit_dropout_path_rate,
        type=float,
        help="drop path rate",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--roi_x",
        default=config.patch_shape[0],
        type=int,
        help="roi size in x direction",
    )
    parser.add_argument(
        "--roi_y",
        default=config.patch_shape[1],
        type=int,
        help="roi size in y direction",
    )
    parser.add_argument(
        "--roi_z",
        default=config.patch_shape[2],
        type=int,
    ...
    parser.add_argument("--datasets", default="PretrainDatasetCT")
    parser.add_argument(
        "--json_list",
        default=None,
        type=str,
    ...
    args.logdir = os.path.join(
        args.logdir,
        f"gmm_{args.basic_mask_ratio}_{args.scale}"
